...
get_ipython().system('jupyter nbconvert --to script plot_organoid_surface_bead_centroid_distance.ipynb')
...
# OUTPUT BASE FOLDER
output_base = Path("/NAS2/Nikita/Plot_centroid_surface_ditance/surface_distance_plot")
...
for input_file in csv_files:
    ...
    # Extract only FOV number (e.g., 00_02_00)
    ...
    for organoid_id, organoid_df in df.groupby("organoid_id"):
        ...
        ax1.set_ylabel("Surface Distance (µm)")
        ...
        # SECOND Y AXIS (exact same scale)
        ax2 = ax1.twinx()
        ax2.set_ylim(ax1.get_ylim())
        ax2.set_yticks(ax1.get_yticks())
        ax2.set_ylabel("Organoid Radius (µm)")

        # Radius line
        radius_df = organoid_df.drop_duplicates(subset=["frame"])
        ...
        plt.title(f"{file_id}_Organoid Surface-Bead Centroid Distance - organoid {int(organoid_id)}")
