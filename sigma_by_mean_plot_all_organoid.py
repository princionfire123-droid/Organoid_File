...
get_ipython().system('jupyter nbconvert --to script sigma_by_mean_plot_all_organoid.ipynb')
...
# -------- PATHS --------
input_root = Path("/NAS2/Nikita/sigma_by_mean")
output_root = Path("/NAS2/Nikita/sigma_oover_beads_organoid_nikita")
...
for csv_file in input_root.rglob("*.csv"):
    try:
        fov = extract_fov(csv_file.parents[1].name)
        org = extract_organoid(csv_file.parent.name)

        if fov not in selection or org not in selection[fov]:
            continue

        df = pd.read_csv(csv_file)

        if "sigma_over_mean" not in df.columns:
            continue

        # 🔥 NEW: use sigma_over_mean mean
        organoid_mean = df["sigma_over_mean"].mean()
...
plt.xlabel("Beads")
plt.ylabel("σ / Mean")
plt.title("Relative Variability (σ/Mean) of Beads Across Organoids")

plt.xticks([])

plt.tight_layout()

output_file = output_root / "sigma_over_mean_sorted.png"
